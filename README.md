...
- 노선 등록 시 상행 종점역과 하행 종점역, 구간 거리, 소요 시간을 입력받는다.
...
- 이미 존재하는 구간은 등록할 수 없다.
  - 교대 - 강남 - 역삼 구간이 등록되어 있다면 교대 - 역삼 구간은 등록할 수 없다.
- 기존의 구간 사이에 새로운 구간을 등록할 경우 구간을 이어줘야 한다.
  - 교대 - 역삼 구간이 등록되어 있는데 교대 - 강남 구간을 등록하면 교대 - 강남 - 역삼 구간으로 수정되어야 한다. 즉 강남 - 역삼 구간이 자동적으로 등록된다.
  - 위와 같은 상황에서 자동으로 추가되는 구간(강남 - 역삼)은 거리 2km, 시간 3분으로 고정되어 추가된다.
- 노선에서 갈래길은 생길 수 없다.
...
- 노선에 등록된 역을 제거할 수 있다.
- 종점을 제거할 경우 다음(또는 이전) 역이 종점이 된다.
- 노선에 포함된 역이 두개 이하일 때는 역을 제거할 수 없다.
- 구간을 제거할 경우 기존에 걸렸던 시간을 구간에 적용한다.
  - 교대 - ( 2km / 3분 ) - 강남 - ( 2km / 3분 ) - 역삼 구간에서 교대 - 강남 구간을 삭제하면 교대 - (4km / 6분) - 역삼 구간으로 변경된다.
- 직접 연결된 구간이 아닐 경우(구간 사이에 다른 역이 존재할 경우) 삭제가 불가능하다.
...
> 위와 같은 스타일에서 일부 수정을 하겠습니다.
>
> 상행 종점과 하행 종점, 구간 거리와 소요 시간까지 입력해서 노선을 등록합니다.
...
> 위와 같은 스타일에서 일부 수정을 하겠습니다.
>
> `구간 관리 > 수정할 노선`을 클릭하면 **역**과 **순서**를 입력하는 것이 아니라, **상행역, 하행역, 구간 거리, 소요 시간**을 입력하여 등록합니다.
>
> 상행역 선택 : 역 선택(ex. 역삼역)
>
> 하행역 선택 : 역 선택(ex. 잠실역)
...
> 소요 시간(분) : 시간 입력(ex. 3)
>
> 상행역, 하행역 선택시에 드롭다운 리스트의 형식은 그대로 유지합니다.
>
> 구간 거리, 소요 시간의 경우 순서를 입력하는 경우와 같이 화살표로 숫자를 조절하는 형식을 그대로 유지합니다.
...
> 위와 같은 스타일에서 일부 수정을 하겠습니다.
>
> 노선에 해당하는 구간 사이에 구간 거리와 소요 시간을 함께 출력합니다.
>
> 2호선
>
> - 교대
